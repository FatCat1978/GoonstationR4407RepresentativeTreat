...
/client/verb/yomusic(var/g as sound)
	set category = "MUSIC"
	set name = "Play Music"
	...
	if (!src.authenticated || !src.holder)
		...
		return
	world << sound(g,channel=768)

/client/verb/yomusicNO()
	set category = "MUSIC"
	set name = "Stop Music"
...
world
	New()
		..()
		spawn(100)
			world << "GENERATING BULLSHIT (SANDBOX SHOULD WORK NOW)"
			for(var/i in typesof(/obj)+typesof(/mob))
				listofitems = "[listofitems]<br><a href=?[i]>[i]</a>"

/client/verb/spawn_atom2()
	set category = "SANDBOX"
	set name="Spawn Literally Whatever You Want"

	var/output = "[listofitems]"
	src << "browsing"
	src << browse(output,"window=sandboxwindow;size=800x600;can_close=1")

/client/verb/delete_this(atom/movable/O as obj|mob in world)
	set category = "Debug"
	set name = "Delete THIS"
	var/cando = 1
	if(istype(O,/mob))
		var/mob/E = O
		if(E.client)
			cando = 0
	if(cando == 0)
		src << "<font color='red'>Failed to delete. This may happen if the object is player controlled."
		return
