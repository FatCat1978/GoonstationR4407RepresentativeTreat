...
/obj/disposalpipe
	...
	proc/expel(var/obj/disposalholder/H, var/turf/T, var/direction)
		...
		if(T.intact && istype(T,/turf/simulated/floor)) //intact floor, pop the tile
			...
			F.intact	= 0
			F.levelupdate()
			new /obj/item/weapon/tile(H)	// add to holder so it will be throw2n with other stuff
		...
		if(direction)		// direction is specified
			...
			for(var/atom/movable/AM in H)
				...
				spawn(1)
					if(AM)
						AM.throw2_at(target, 100, 1)
			H.vent_gas(T)
			del(H)

		else	// no specified direction, so throw2 in random direction
...
/obj/disposaloutlet
	...
	anchored = 1
	var/active = 0
	var/turf/target	// this will be where the output objects are 'throw2n' to.
